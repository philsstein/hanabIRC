...
class Player(object):
	...
	def get_hand(self, hidden=False):
		if not hidden:
			return '%s: %s' % (self.markup.markup(self.name, self.markup.BOLD), ''.join([str(c) for c in self.hand]))
		else:
			return str(self)

	def __str__(self):
			return '%s: %s' % (self.markup.markup(self.name, self.markup.BOLD), ''.join(['?' for c in self.hand]))
...
class Game(object):
	'''
		...
		In this example the "..." would be the random hand of the player listed. 
	'''
	def __init__(self, name, markup):
		'''
		name: the name of the game as an ID.
		text_markup: a class for marking up the the return game state text. Enables
			bolding, colorizing, etc text that is returned. Useful for abstracting
			markup to support mulitple bolding and colorizing contexts, like IRC and 
			xterms. 
		'''
		self.players = []
		self.max_players = 5
		...
		colors = [markup.RED, markup.WHITE, markup.BLUE, markup.GREEN, markup.YELLOW]
		self.deck = [Card(c, n, self.markup) for c in colors for n in [1, 1, 1, 2, 2, 2, 3, 3, 4, 4, 5]]
	...
	def get_status(self, nick, show_all=False):
		...
		for p in self.players:
			if show_all:
				retVal.append(p.get_hand())
			else:
				if p.name != nick:
					retVal.append(p.get_hand())
				else:
					retVal.append(p.get_hand(hidden=True))
		
		retVal.append('Notes: %s, Storms: %s' % (''.join(self.notes), ''.join(self.storms)))
		retVal.append('%d cards remaining' % len(self.deck))
		
		return retVal

	def show_game_state(self):
		'''Show the entire game status - only for debugging/super user/non-players.'''
		retVal = list()
		if len(self.players):
			retVal += self.get_status(self.players[0].name, show_all=True)
		else:
			retVal.append('Game is waiting for players to join')
	...
	def add_player(self, nick):
		...
		if len(self.players) > self.max_players:
			retVal.append('max players already in game %s. You can start another with !new [name]' % self.name)
		else:
			for p in self.players:
				if p.name == nick:
				...
				retVal.append('You have joined game %s' % self.name)
	...
	def remove_player(self, nick):
		...
		for p in self.players:
			if p.name == nick:
				retVal.append('Putting %s\'s cards back in the deck and reshuffling.' % (p.name))
				self.deck += p.hand
				random.shuffle(self.deck)
