...
class Hanabot(SingleServerIRCBot):
	...
	def parse_commands(self, event, cmds):
		...
		if not cmds[0] in self.commands:
			self.connection.notice(nick, 'My dearest brother \x035Willis\x03, I do not understand this "\x036,8%s\x03" of which you speak.' % ' '.join(cmds))
			return
	...
	def handle_status(self, args, event):
		...
		for g in self.games:
			self.connection.notice(nick, '%s' % g)
	...
	def handle_new(self, args, event):
		...
		if game in self.games.keys():
			self.connection.notice(event.source.nick, 'The game %s already exists.' % game)
		else:
			log.info('Starting new game %s' % game)
			self.games[game] = Game(self.connection, game, self.channel)
			self.connection.notice(self.channel, 'New game "%s" started. Accepting joins now.' % game)
