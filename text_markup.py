...
class text_markup_base(object):
	...
	def markup(self, text, markup):
		'''just check for supported markup. raise exception if not supported.'''
...
class irc_markup(text_markup_base):
	...
	def markup(self, text, markup):
		...
		# we only do bold for now. Replace with if esif chain when we support more.
		return '\x02%s\x02' % text

	def color(self, text, color):
		text_markup_base.color(self, text, color)
		return '\x03%d%s\x03' % (irc_markup._colormap[color], text)
...
class ascii_markup(text_markup_base):
	...
	>>> a = ascii_markup()
	>>> a.color('hello', ascii_markup.RED)
	'r[hello]'
	>>> a.markup('hello', ascii_markup.BOLD)
	'HELLO'
	>>> a.color('hello', 'purple')
	Traceback (most recent call last):
		...
	text_markup_exception: 'Unknown color: purple'
	>>> a.markup('hello', 'underlined')
	Traceback (most recent call last):
		...
